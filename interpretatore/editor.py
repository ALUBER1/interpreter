...
def salva_nome():
    ...
    with open("messaggi\path.txt",'w')as file_path:
        file_path.write(path)
def cerca():
    input=simpledialog.askstring("cerca","inserisci parola da cercare ")
    p=testo.get("1.0",tkinter.END)
    trova=p.find(input)
    n=0
    for i in range(0,trova):
        if p[i]=='\n':
            n=n+1
    testo.see(str(trova)+"."+str(n))
...
modifica=tkinter.Menu(men,tearoff=0)
modifica.add_command(label="cerca",command=cerca)
men.add_cascade(label="modifica",menu=modifica)
